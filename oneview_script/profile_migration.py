...
def callback(ch, method, properties, body):
        ...
        if(('alertState' in resource) and ('severity' in resource)):
               if((('/rest/server-profiles' in resource['resourceUri']) or ('/rest/server-hardware' in resource['resourceUri'])) and (('Active' == resource['alertState']) or ('Locked' == resource['alertState'])) and ('Critical' == resource['severity']) and ((resource['resourceUri'] not in serverProfilesList) and (resource['resourceUri'] not in serverHardwareList))):
                        if('/rest/server-profiles' in resource['resourceUri']):
                                resourceUri=resource['resourceUri']
                                serverProfilesList.append(resourceUri)
                        if('/rest/server-hardware' in resource['resourceUri']):
                                try:
                                        credential = {'userName': user, 'password': passwd}		
                                        sec = security(con)		
                                        login(con, credential)
                                        acceptEULA(con)
                                        hardwareUri=resource['resourceUri']
                                        serverHardwareList.append(hardwareUri)							
                                        hardwareResource=con.get(hardwareUri)
                                        if(('serverProfileUri' in hardwareResource) and (hardwareResource['serverProfileUri'] not in serverProfilesList)):
                                                resourceUri=hardwareResource['serverProfileUri']
                                                serverProfilesList.append(resourceUri)
                                        else:
                                                resourceUri = None
                                        serverHardwareList.remove(hardwareUri)
                                        logout(con)
                                except:
                                        print("Failure in callback while fetching server profile uri")
                        print(resourceUri)
                        if(resourceUri != None):
...
parser.add_argument('-a', '--appliance', dest='host',default='10.54.31.211', required=False,
